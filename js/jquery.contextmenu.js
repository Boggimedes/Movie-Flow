	...
	$.contextMenu = {
		...
		show: function(t,e) {
			...
			if (cmenu.beforeShow()!==false) {
				ctarget = cmenu.target;
				var index = ctarget.children[0].src.lastIndexOf('/');
				if(index != -1)
				{
					var newStr = ctarget.children[0].src.substring(index + 1);
				}
				console.log(newStr.replace(".jpg",""));
				var mData = (function() {
				var mData = null;
				$.ajax({
				'cache': false,
				'async': false,
				'global': false,
				'url': 'getdata.php?movie=' + newStr.replace(".jpg",""),
				'dataType': "json",
				...
				document.getElementById('Title').value = mData.Title;
				document.getElementById('Year').value = mData.Year;
				document.getElementById('Runtime').value = mData.Runtime;
				document.getElementById('Rating').value = mData.Rating;
				document.getElementById('Description').innerHTML = mData.Description;
				document.getElementById('Path').value = mData.Path;

		try
  {
		console.log(mData.Genres.Genre);
		console.log(mData.Genres);
		vartest = mData.Genres;
		selectableArray = mData.Genres;
		console.log(selectableArray);
		selectableArray = selectableArray.replace("SD","21");
		selectableArray = selectableArray.replace("HD","20");
		selectableArray = selectableArray.replace("Western","19");
		selectableArray = selectableArray.replace("War","18");
		selectableArray = selectableArray.replace("Thriller","17");
		selectableArray = selectableArray.replace("Sci-Fi","16");
		selectableArray = selectableArray.replace("Romance","15");
		selectableArray = selectableArray.replace("Mystery","14");
		selectableArray = selectableArray.replace("Musical","13");
		selectableArray = selectableArray.replace("Music","12");
		selectableArray = selectableArray.replace("Horror","11");
		selectableArray = selectableArray.replace("History","10");
		selectableArray = selectableArray.replace("Fantasy","9");
		selectableArray = selectableArray.replace("Family","8");
		selectableArray = selectableArray.replace("Drama","7");
		selectableArray = selectableArray.replace("Crime","6");
		selectableArray = selectableArray.replace("Comic-Book","5");
		selectableArray = selectableArray.replace("Comedy","4");
		selectableArray = selectableArray.replace("Animation","3");
		selectableArray = selectableArray.replace("Adventure","2");
		selectableArray = selectableArray.replace("Action","1");	
		selectableArray = selectableArray.replace("Favorites","0");
		console.log(selectableArray);
...
try
  {
liArray = selectableArray.split(", ");

			$('#selectable2 li').removeClass('ui-selected');

			for(var i = 0; i <= liArray.length; i++){
			$('#selectable2 li:eq('+liArray[i]+')').addClass('ui-selected');
		}
		Q1 = ", " + selectableArray;
  }
catch(err)
  {
				...
				cmenu.shown=true;
				$('.folder').one('click',null,function(){cmenu.hide()}); // Handle a single click to the document to hide the menu
			}
		},
